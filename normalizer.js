...
function normalizeMarket(marketName, marketData, sport, home, away) {
  ...
  for (const [rawKey, rawVal] of Object.entries(marketData)) {
    ...
    if (totalMatch) {
      ...
      // For Soccer & Handball Match / Full Time Result, preserve standard 1 / X / 2 labeling
      if (mName === 'match result' || mName === 'full time result') {
        sel = rawKey; // '1', 'X', '2'
      } else if ((mName === 'match winner' || mName === 'to win match' || mName === 'first set winner') && (sport.includes('Tennis') || sport === 'Tennis')) {
  ...
  } else if (mName === 'to win match' || mName === 'first set winner' || mName === 'match winner') {
    const order12 = { '1': 1, '2': 2 };
    items.sort((a, b) => (order12[a.rawKey] || 99) - (order12[b.rawKey] || 99));
  } else if (mName.includes('spread') || mName.includes('handicap')) {
    const orderSpread = { '1': 1, '2': 2 };
    items.sort((a, b) => (orderSpread[a.rawKey] || 99) - (orderSpread[b.rawKey] || 99));
  } else if (OUTRIGHT_SPORTS.has(sport) || !away) {
