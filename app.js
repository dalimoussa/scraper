...
(function () {
  ...
  /* ==========================================================================
     ...
     Markets:
     - Match Winner: 1 [odds] | 2 [odds]
     - Set Betting: Player 1 2-0, 2-1, Player 2 2-0, 2-1
  ...
  function renderTennisCard(ev) {
    ...
    // Match Winner (1 | 2)
    const mwKey = markets['Match Winner'] ? 'Match Winner' : (markets['To Win'] ? 'To Win' : null);
    if (mwKey && markets[mwKey]) {
      ...
      for (const item of markets[mwKey]) {
        group.appendChild(createOddsButton(ev, 'Match Winner', item));
      }
      section.appendChild(group);
      card.appendChild(section);
    }
  ...
  /* ==========================================================================
     ...
     Markets:
     - Spread: Team 1 +1.5 — odds, Team 2 -1.5 — odds (Line + Odds)
     - Money Line: Team 1 — odds, Team 2 — odds (Simple Odds)
     - Handicap: Team 1 +X — odds, Team 2 -X — odds (Line + Odds)
  ...
  function renderBasketballCard(ev) {
    ...
    // Spread or Handicap
    const spreadKey = markets['Spread'] ? 'Spread' : (markets['Handicap'] ? 'Handicap' : null);
  ...
  /* ==========================================================================
     9. HANDBALL RENDERER
     Markets:
     - Money Line / Match Winner: Team 1 — odds, Team 2 — odds (Simple Odds)
  ...
  function renderHandballCard(ev) {
    ...
    const markets = ev.markets;
    const mlKey = markets['Money Line'] ? 'Money Line' : (markets['Match Winner'] ? 'Match Winner' : (markets['To Win'] ? 'To Win' : null));

    if (mlKey && markets[mlKey]) {
      const section = document.createElement('div');
      section.className = 'market-section';
      section.innerHTML = `<div class="market-label">Match Winner / Money Line</div>`;
      const group = document.createElement('div');
      group.className = 'odds-group grid-2';
      for (const item of markets[mlKey]) {
        group.appendChild(createOddsButton(ev, mlKey, item));
  ...
  function renderOutrightCard(ev) {
    ...
    // Find the primary outright market (To Win, To Win Outright, Race Winner)
    const primaryKey = Object.keys(ev.markets).find(k => 
      k.toLowerCase().includes('win') || k.toLowerCase().includes('winner')
    ) || Object.keys(ev.markets)[0];

    const items = primaryKey ? (ev.markets[primaryKey] || []) : [];
      ...
      <div class="outright-title-group">
        ...
        <div>
          ...
          <div class="outright-meta">
            <span>📅 ${escapeHtml(ev.kickoff || ev.date || '')}</span>
            <span>🏁 ${items.length} Contenders</span>
    ...
    // Contenders Filter Bar
    const filterBar = document.createElement('div');
    filterBar.className = 'outright-filter-bar';
    filterBar.innerHTML = `
      <div style="font-size:0.8rem; font-weight:700; color:var(--text-dim); text-transform:uppercase;">
        Market: ${escapeHtml(primaryKey || 'Outright Winner')}
      </div>
    ...
    const renderItems = (filterText = '') => {
      ...
      for (const item of matching) {
        grid.appendChild(createOddsButton(ev, primaryKey, item));
      }
    };
